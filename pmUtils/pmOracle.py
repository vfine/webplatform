...
$Id: pmOracle.py 18226 2014-02-04 20:28:19Z fine $
"""
...
class sqlSelect(object):
  ...
  #______________________________________________________________________________________________________________
   ...
   #______________________________________________________________________________________________________________
   def add(self,variable,value=None, ops='=',refs=None):
      ...
      elif isinstance(value,str) and value.lower() == 'any':
         pass
      elif isinstance(value,str) and ( value.lower() == 'unknown' or value.lower() == '*unknown'):
         value = 'NULL'
         if ops ==  '=': ops = ' IS  '
         if ops == '!=': ops = ' IS NOT '
         where.append('%(var)s %(ops)s %(val)s'% {'var' : variable, 'ops': ops, 'val':value} )
  ...
  #______________________________________________________________________________________________________________
   ...
   def prepareJobType(self,jobtype,testsite=False):
      ...
      if not hasJedi:      
         if testsite==True:
            prodtype = labels.get('prodSourceLabel',[])
            if hasProduction:
               if  not 'prod_test' in prodtype:
                  prodtype += ['prod_test']
            elif not '*test' in prodtype:
               if not '*test' in prodtype:
                  prodtype += ['*test']
               self.add('prodSourceLabel','prod_test','!=')
            labels['prodSourceLabel'] = prodtype
         elif hasProduction:
   ...
   #______________________________________________________________________________________________________________
   def prepareConditions(self,conditions,exceptions=None,forceregexp=False):
      if conditions != None:
         for c in conditions:
            if not c.upper() in exceptions:
               self.prepareCommaList(c,conditions[c])
   #______________________________________________________________________________________________________________
      ...
      if ( value==None or value =='None' )and isinstance(key,dict):
         ...
         if not self.is_all(value): 
            ...
            if isinstance(value,list):
               ...
               vstar = '%s' % value
               if vstar.lower() == 'null' or vstar.lower() == 'unknown':
