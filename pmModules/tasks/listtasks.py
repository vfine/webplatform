"""Monitor the Selected Tasks </td><td>$Rev: 18208 $"""
...
class listtasks(pmModule):
   """ List the Selected Tasks </td><td>$Rev: 18208 $"""

   #______________________________________________________________________________________
   def __init__(self,name=None,parent=None,obj=None):
      pmModule.__init__(self,name,parent,obj)
      self._table='T_PRODUCTION_TASK'
      self._description,params = utils.normalizeDbSchema(pdf,self._table)
      self._allparams = ','.join(params.keys())      
      
      self._tpattern = re.compile('([^\.]+)\.([^\.]+)\.([^\.]+)\.([^\.]+)\.(.*)')
      self._tagpattern = re.compile('(.*)_([^_]+)$')
      #                              SQL                    Web
      self._select = (   ('taskID',                      'Id'     )
                       , ('taskname',                   'Task_Name')
   ...
   def parseTaskName(self, taskName):
      ...
      if tn:
         ...
         return None        
   #______________________________________________________________________________________      
   def doJson(self,hours=None,days=None,tidm=None,name=None,dset=None,project=None,columns=None,phys_ref=None,step=None,tag=None,dsn=None,ctag=None,failed=None,state=None,formats=None,action=None,jobstatus=None):
      """ 
        Requested Tasks
        <ui>
        ...
        <li><a title='Click to see the full list of the listtask1 parameters available' href='http://pandamon.cern.ch/describe?table=^T_PRODUCTION_TASK$&db=pdf'><b>PARAM</b></a>=value</td><td>-  comma-separated list of the <a href ='https://twiki.cern.ch/twiki/bin/view/PanDA/PandaPlatform#API_parameter_value_format'> patterns</a> to filter the job records.<br>
        <hr>
      ...
      if tidm != None:
         ...
         if len(tidarray) > 1: 
             tidm=None
             tid = tidarray
      q = pdf.listTaskRequests(tidm=tidm,hours=hours,taskname=name,dset=dset,select=select,project=project,physics=phys_ref,stage=step,tag=tag,dsn=dsn,ctag=ctag,tid=tid,failed=failed,state=state,formats=formats,conditions=vals,table='ATLAS_DEFT.%s' %self._table)
      header = q['header']
      rows   = q['rows']
      ...
      self.publishTitle('Task Requests')
      self.publish(main)
      self.publish( "%s/%s" % (self.server().fileScriptURL(),"taskdef/%s.js" % 'listTasks' ),role=pmRoles.script())
